import { getActiveBrand, getAllBrands, createBrand, activateBrand, updateBrand, deleteBrand } from "../models/siteBrand.model.js";
...
export const updateBrandCtrl = async (req, res) => {
  try {
    ...
    const logoPath = req.file ? `/uploads/brand/${req.file.filename}` : null;
    await updateBrand(id, logoPath, brand_name.trim());
    const brands = await getAllBrands();
    res.json({ success: true, data: brands });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
